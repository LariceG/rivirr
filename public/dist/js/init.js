...
var droopy = function(){
	...
	$(document).on("keypress","#input_msg_send",function (e) {
		if ((e.which == 13)&&(!$('#input_msg_send').val().length == 0)) {
			var today = new Date();
			...
			$.ajax({
				type:"POST",
				url: $('#url').val(),
				data:{message: $(this).val(),receiver_id: $('#receiver_id').val(), "_token": $('#_token').val()},
				success:function(result)
				{
					$('<li class="self mb-10"><div class="self-msg-wrap"><div class="msg block pull-right">' + $('#input_msg_send').val() + '<div class="msg-per-detail mt-5"><span class="msg-time txt-grey">'+dateTime+'</span></div></div></div><div class="clearfix"></div></li>').insertAfter(".fixed-sidebar-right .chat-content  ul li:last-child");
					$('#input_msg_send').val('');
				}
			 });  
		} else if(e.which == 13) {
			alert('Please type somthing2!');
		}
		return;
	});
